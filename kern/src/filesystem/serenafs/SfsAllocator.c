...
{
    decl_try_err();
    const uint32_t volumeBlockCount = be32toh(vhp->volBlockCount);
    const uint32_t allocationBitmapByteSize = be32toh(vhp->allocBitmapByteSize);
    FSBlock blk = {0};
    ...
    self->bitmapLba = be32toh(vhp->lbaAllocBitmap);
