...
open_class_funcs(Filesystem, Object,
    ...
    // Creates a new inode with type 'ftype', user information 'user',
    // permissions 'fsperms' and adds it to the directory 'pDir'. The new
    // node will be added to 'pDir' with the name 'pName'. Returns the newly
    // acquired inode on success and NULL otherwise.
    errno_t (*createNode)(void* _Nonnull self, InodeRef _Nonnull _Locked pDir, const PathComponent* _Nonnull pName, DirectoryEntryInsertionHint* _Nullable pDirInsertionHint, uid_t uid, gid_t gid, fs_ftype_t ftype, fs_perms_t fsperms, InodeRef _Nullable * _Nonnull pOutNode);
...
#define Filesystem_CreateNode(__self, __pDir, __pName, __pDirInsertionHint, __uid, __gid, __ftype, __fsperms, __pOutNode) \
invoke_n(createNode, Filesystem, __self, __pDir, __pName, __pDirInsertionHint, __uid, __gid, __ftype, __fsperms, __pOutNode)
