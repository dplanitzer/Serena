...
errno_t GraphicsDriver_ioctl(GraphicsDriverRef _Nonnull self, unsigned int mode, off_t* _Nonnull pOffset, int cmd, va_list ap)
{
    switch (cmd) {
        ...
        default:
            return super_n(ioctl, Driver, GraphicsDriver, self, mode, pOffset, cmd, ap);
    }
}
