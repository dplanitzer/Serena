...
errno_t Console_InitVideo(ConsoleRef _Nonnull self)
{
    ...
    try(GraphicsDriver_SetSpritePixels(self->gdevice, self->textCursor, textCursorPlanes));
    ...
    try(GraphicsDriver_MapSurface(self->gdevice, self->surfaceId, kMapPixels_ReadWrite, &self->pixels));
