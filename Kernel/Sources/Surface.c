...
{
    Surface* pSurface = (Surface*) kalloc(sizeof(Surface), HEAP_ALLOC_OPTION_CLEAR);
    FailNULL(pSurface);
    ...
    for (Int i = 0; i < pSurface->planeCount; i++) {
        pSurface->planes[i] = kalloc(bytesPerPlane, HEAP_ALLOC_OPTION_CPU|HEAP_ALLOC_OPTION_CHIPSET);
