...
#define IOACTION_PUBLISH    1
#define IOACTION_UNPUBLISH  2

typedef void (*drv_match_func_t)(void* _Nullable arg, HandlerRef _Nonnull driver, int action);
...
// Iterates through all currently published drivers and checks which ones match
// at least one of the I/O categories 'cats'. The array 'buf' is filled with
// strong references to the matching drivers. The number of driver references
...
// driver matching any of the I/O categories 'cats' is published or unpublished.
// The function 'f' is also invoked with each driver that is already published
