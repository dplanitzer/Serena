...
errno_t HIDManager_ObtainCursor(HIDManagerRef _Nonnull self)
{
    decl_try_err();

    mtx_lock(&self->mtx);
    if (self->fb) {
        err = GraphicsDriver_ObtainCursor(self->fb);
        if (err == EOK) {
            self->cursorWidth = 0;
            self->cursorHeight = 0;
            self->hotSpotX = 0;
...
void HIDManager_ReleaseCursor(HIDManagerRef _Nonnull self)
{
    mtx_lock(&self->mtx);
    if (self->fb) {
        GraphicsDriver_ReleaseCursor(self->fb);
...
errno_t HIDManager_SetCursor(HIDManagerRef _Nonnull self, const void* _Nullable planes[], size_t bytesPerRow, int width, int height, PixelFormat format, int hotSpotX, int hotSpotY)
{
    ...
    try(GraphicsDriver_BindCursor(self->fb, self->cursorSurfaceId));
    ...
    GraphicsDriver_SetCursorPosition(self->fb, self->mouse.x - hotSpotX, self->mouse.y - hotSpotY);
...
static bool _show_cursor(HIDManagerRef _Nonnull _Locked self)
{
    ...
    if (self->hiddenCount == 0) {
        GraphicsDriver_SetCursorPosition(self->fb, self->mouse.x - self->hotSpotX, self->mouse.y - self->hotSpotY);
        GraphicsDriver_SetCursorVisible(self->fb, true);
        return true;
    }
    else {
        return false;
...
{
    if (self->hiddenCount == 0) {
        GraphicsDriver_SetCursorVisible(self->fb, false);
    }
