...
enum {
    ...
    SC_disp_schedule,       // errno_t disp_schedule(int od, os_disp_func_t _Nonnull func, void* _Nullable ctx, uint32_t options, uintptr_t tag)
    SC_vmalloc,             // errno_t vmalloc(int nbytes, void **pOutMem)
    SC_exit,                // _Noreturn exit(int status)
    SC_spawn,               // errno_t spawn(cost char* _Nonnull path, const char* _Nullable argv[], os_spawn_opts_t * _Nullable options, pid_t * _Nullable rpid)
    SC_getpid,              // pid_t getpid(void)
    SC_getppid,             // pid_t getppid(void)
    SC_getpargs,            // os_pargs_t * _Nonnull getpargs(void)
    SC_open,                // errno_t open(const char * _Nonnull name, int options, int* _Nonnull fd)
    SC_close,               // errno_t close(int fd)
    SC_waitpid,             // errno_t waitpid(pid_t pid, os_pstatus_t * _Nullable result)
    ...
    SC_setcwd,              // errno_t setcwd(const char* _Nonnull path)
    SC_getuid,              // uid_t getuid(void)
    SC_getumask,            // FilePermissions getumask(void)
    SC_setumask,            // void setumask(FilePermissions mask)
    ...
    SC_disp_timer,          // errno_t disp_timer(int od, TimeInterval deadline, TimeInterval interval, os_disp_func_t _Nonnull func, void* _Nullable ctx, uintptr_t tag)
