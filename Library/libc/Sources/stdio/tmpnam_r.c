...
char *__tmpnam_r(char *filename, int* pOutIoc)
{
    ...
    if (File_CheckAccess(dir, kAccess_Readable | kAccess_Searchable) != EOK) {
        return NULL;
    }
    ...
    for (int i = 0; i < TMP_MAX; i++) {
        ...
        *p = '\0';
        ...
        if (pOutIoc) {
            const errno_t err = File_Create(filename, kOpen_ReadWrite | kOpen_Exclusive, FilePermissions_MakeFromOctal(0600), pOutIoc);
            if (err == EOK) {
                return filename;
            }
        }
        else {
            if (File_CheckAccess(filename, kAccess_Exists) == ENOENT) {
                return filename;
            }
        }
    }
