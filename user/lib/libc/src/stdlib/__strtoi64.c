...
int __strtoi64(const char * _Nonnull _Restrict str, char * _Nonnull _Restrict * _Nonnull _Restrict str_end, int base, long long * _Nonnull _Restrict result)
{
    ...
    const unsigned long long up_bound = (sig_ch == '-') ? -(unsigned long long)LLONG_MIN : LLONG_MAX;
    const unsigned long long ovf_num = up_bound / b;
    const unsigned long ovf_d = up_bound % b;
    ...
    if (overflowed) {
        *result = (sig_ch == '-') ? LLONG_MIN : LLONG_MAX;
        r = ERANGE;
    }
    else {
