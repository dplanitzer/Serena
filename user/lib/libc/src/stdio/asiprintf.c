...
#ifdef __VBCC__
int __v2asprintf(char **str_ptr, const char * _Nonnull _Restrict format, ...)
{
    va_list ap;
    
    va_start(ap, format);
    const int r = vasiprintf(str_ptr, format, ap);
    va_end(ap);
    return r;
}
#endif

int asiprintf(char **str_ptr, const char * _Nonnull _Restrict format, ...)
{
    va_list ap;
...
int vasiprintf(char **str_ptr, const char * _Nonnull _Restrict format, va_list ap)
{
    ...
    const int r1 = __fmt_print(&fmt, format, ap);
