...
// @Concurrency: Safe
extern errno_t sem_timedwait(sem_t* _Nonnull self, int flags, const nanotime_t* _Nonnull wtp);

// Attempts to acquire one permit without blocking. Returns 0 on success and -1
// and errno set to EAGAIN otherwise.
// @Concurrency: Safe
extern errno_t sem_trywait(sem_t* _Nonnull sema);
