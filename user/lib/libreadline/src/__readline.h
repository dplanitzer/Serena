...
struct readline {
    ...
    // Input line
    char* _Nonnull  line;               // has an extra character for the terminating NUL
    int             lineCapacity;
    ...
    // History buffer
    char* _Nullable savedLine;  // Line saved if 'line' was dirty when user hits crsr-up/down
    bool            isDirty;

    char** _Nullable    history;
