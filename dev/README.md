...
This folder contains the sources for tools that are needed to build Serena OS from scratch on a host system like Windows, macOS or Linux. All the tools except diskimage can be built on either Windows or POSIX compatible hosts. Note however that the diskimage tool currently builds on Windows only.

Invoke the make file in this directory to build all tools. You will then be able to find the compiled executables in the `build/host` folder.
